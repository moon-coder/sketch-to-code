...
import {calcBoundaryNode, isContainer, calcNodeCoordsNew} from '../utils';
...
const preDeal = (nodes: INode[]): INode => {
  ...
  const addToOuter = (node: INode, nodes: INode[]): boolean => {
    ...
    const outers = nodes.filter(
      item => item.type === 'Block' && item != node && isContainer(node, item),
    );
    ...
    // if(outers[0]?.__layer?.name==='矩形该死的地方'){
    //   debugger;
    // }
...
const mergeLineRow = (node: INode) => {
  ...
  if (
    node.parent &&
    ((node.parent.style.flexDirection &&
      node.parent.style.flexDirection == node.style.flexDirection &&
      !hasVisualKey(node)) ||

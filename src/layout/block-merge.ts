...
function getRelPosition(node1:INode,node2:INode) {
  ...
  let xRel =lineCompare({beg:x1,end:x1+width1},{beg:x2,end:x2+width2});
  let yRel =lineCompare({beg:y1,end:y1+height1},{beg:y2,end:y2+height2});
 ...
 */
function getMinAreaMergeNode(targetNode:INode,mergeableNodes:INode[]):{
  adapterNode:INode,others:INode[]
} {
  let minArea =null,minNodeIndex:number=0;

  for (let i = 0, iLen = mergeableNodes.length; i < iLen; i++) {
    let mergeableNode = mergeableNodes[i];
    let {frame:{width,height}}= calcBoundaryNode([mergeableNode,targetNode]);
  ...
  return {
    adapterNode:adapterNode[0],others:mergeableNodes
  }
 ...
 */
function getMergeableNodes(targetNode:INode,restNodes: INode[]):{
  mergeable:INode[],unMergeable:INode[]
}{
  let mergeable :INode[]= [] ,unMergeable :INode[]= [];
 ...
 */
function isOverLapThird(first: INode, second: INode,restNodes:INode[]):boolean {
